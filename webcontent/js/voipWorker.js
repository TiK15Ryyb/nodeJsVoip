    ...
    if (inc) {
      ...
      resample(
        ...
        function(resapledData) {
          ...
          if (minGain != null) resapledData = gainGuard(resapledData, minGain); //Set resapledData to null if voice is to low
...
function upSample(fromSampleRate, toSampleRate, chunkSize, buffer, callBack) {
  ...
  var s =
    buffer.length > 0
      ? Smooth(buffer, { scaleTo: outLength, method: "cubic", clip: "clamp" })
      : [];
  ...
  */
  ...
  AbstractInterpolator = (function() {
    function AbstractInterpolator(array, config) {
      if (typeof array.slice === "undefined") this.array = array.subarray(0);
